...
插件通过 **SessionStart 钩子**在每个会话启动时注入编码触发症状和 **1% 规则**，主动提醒模型——而非被动等待模型记住这个技能。

- 会话启动时注入 `<EXTREMELY_IMPORTANT>` 块，包含乱码检测症状、1% 规则（"如果存在任何编码症状，必须加载技能"）、以及拦截常见编码合理化借口的 Red Flags（"看起来没问题"、"Set-Content 一直没事"、"快速一行命令不用管"）
- `Skill("powershell-safety")` — 加载核心安全规则和症状→修复速查表
